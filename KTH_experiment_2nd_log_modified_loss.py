...
def l2_reconstruction_loss(x, x_, foreground_mask=None, epsilon=1e-6):
    ...
    if foreground_mask is not None:
        ...
        # Calculate logarithmic weighting factor: log[1/(1-saliency)]
        # Clamp (1-foreground_mask) to avoid log(0)
        inverse_background = torch.clamp(1.0 - foreground_mask, min=epsilon)
        log_weight = torch.log(1.0 / inverse_background)
        
        # Apply logarithmic weighting to loss
        weighted_loss = pixel_loss * log_weight
        
        # Normalize by mean weight to maintain reasonable scale
